...
const isCordsValid = ({ x, y }) => {
  const isCordBetweenRange = (cord) =>
    MIN_CORD_RANGE <= cord && cord <= MAX_CORD_RANGE;
...
const isShipPlaceable = (start, end, board, isVertical) => {
  ...
  if (isVertical) {
    ...
    // check the top side
    if (
      isCordsValid({ x: start.x, y: start.y - 1 }) &&
      board[start.x][start.y - 1] === "s"
    ) {
      return false;
    }
    ...
    if (
      isCordsValid({ x: start.x, y: end.y + 1 }) &&
      board[start.x][end.y + 1] === "s"
    ) {
      return false;
    }
  ...
  // check the left side
  if (
    isCordsValid({ x: start.x - 1, y: start.y }) &&
    board[start.x - 1][start.y] === "s"
  ) {
    return false;
  }
...
const isShipValid = (ship, board, { x, y, isVertical }) => {
  if (ship && isCordsValid({ x, y })) {
    ...
    return (
      isCordsValid(endPoint) &&
      isShipPlaceable(startPoint, endPoint, board, isVertical)
    );
  }
...
const isBorderForAnotherShip = (board, { x, y }) => {
  ...
  if (isCordsValid({ x: x - 1, y: y - 1 }) && board[x - 1][y - 1] === "s")
    return true;
  if (isCordsValid({ x: x + 1, y: y - 1 }) && board[x + 1][y - 1] === "s")
    return true;
  if (isCordsValid({ x, y: y + 1 }) && board[x][y + 1] === "s") return true;
...
const removeShipFromBoard = (shipData, board) => {
  ...
  if (isVertical) {
    ...
    if (isCordsValid({ x: startPoint.x, y: startPoint.y - 1 })) {
      if (
        !isBorderForAnotherShip(newBoard, {
          x: startPoint.x,
          y: startPoint.y - 1,
        })
      ) {
  ...
  if (isCordsValid({ x: endPoint.x + 1, y: startPoint.y })) {
    if (
      !isBorderForAnotherShip(newBoard, { x: endPoint.x + 1, y: startPoint.y })
    ) {
...
const getPositionIndex = (ship, { x, y }) =>
  ship.cords.findIndex((c) => c.x === x && c.y === y);
    ...
    receiveAttack({ x, y }) {
      if (this.isReady() && isCordsValid({ x, y })) {
        if (board[x][y] === "s") {
          ...
          if (
            damagedShipData.ship
              .hitAt({ position: damagedPosition + 1 })
              .isSunk()
          ) {
            sunkShips += 1;
            board[x][y] = "x";
            const { newBoard, clearedBorders } = removeShipBorders(
              damagedShipData,
              board
            );
            board = newBoard;
