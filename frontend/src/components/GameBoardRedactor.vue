<template>
  <v-dialog v-model="resetAndDisplay" open-delay="1s" overlay-opacity="0.8" persistent>
    <div class="container">
      <div class="redactor">
        <div class="board-container">
          <div class="cords letters">
            <div v-for="l in lettersCords" :key="l" class="letter">{{ l }}</div>
          </div>
          <div class="cords numbers">
            <div v-for="n in numbersCords" :key="n" class="number">{{ n }}</div>
          </div>
          <div class="board">
            <div
        ...
        <div class="selection">
          ...
          >
            ...
            <div
              ...
              draggable="true"
            >
              <div v-for="i in getShipLength(ship)" :key="i" class="part"></div>
            </div>
          </div>
        </div>
      </div>
      <div class="tips-container">
        ...
        <div class="tips">
          <span>* Перетягуйте кораблі</span>
          <span>* Натисніть на корабель на дошці, щоб змінити напрямок</span>
        </div>
      </div>
      <div class="options">
        <button @click="$router.push('/')">Вийти</button>
        <button @click="handleRandomPlacement">Рандомно</button>
        <button @click="resetBoard()">Скинути</button>
        <button :class="{ disable: totalShips !== 0 }" @click="onClickStartBtn">
          Почати пошук
        </button>
      </div>
    </div>
...
<script>
import createGameBoard, { MAX_CORD_RANGE, MIN_CORD_RANGE, REQUIRED_TYPES_OF_SHIPS } from "../scripts/createGameBoard";
...
export default {
  ...
  methods: {
    onClickStartBtn() {
      ...
      ships.forEach(el => {
        plShips.push({
          X: JSON.parse(el.getAttribute("data-cord"))["x"],
          Y: JSON.parse(el.getAttribute("data-cord"))["y"],
          IsHorizontal: el.getAttribute("data-position") === "x",
          Length: parseInt(el.getAttribute("data-length"))
        });
      });
    ...
    handleShipInitialCord(e) {
      ...
      e.dataTransfer.setData(
        "text/plain",
        JSON.stringify({ cord, cloned: true, position })
      );
    ...
    handleDragStart(e) {
      ...
      e.dataTransfer.setData(
        "text/plain",
        JSON.stringify({ length, position })
      );
    },
    handleDragOver(e) {
    ...
    handleDrop(e) {
      ...
      if (isDataValid && data) {
        if (data.cloned) {
          ...
          const isVertical = position === "x";
          if (
            this.board.placeShipAt(createShip({ length }), {
              ...cord,
              isVertical
            })
          ) {
    ...
    handleRandomPlacement() {
      const renderNewShip = ({ x, y, isVertical }, length) => {
        ...
        [...document.querySelectorAll(".spot")]
          .find((spot) => spot.dataset.cord === JSON.stringify({ x, y }))
          .appendChild(ship);
        ship.addEventListener("dragstart", this.handleShipInitialCord);
        ship.addEventListener("click", this.handleChangePosition);
      };
